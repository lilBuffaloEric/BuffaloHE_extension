...
var (
	Sign7  = SignComposition{Precision: 7, ComponentNum: 2, Components: SignComponentsA7}
	Sign10 = SignComposition{Precision: 10, ComponentNum: 3, Components: SignComponentsA10}
)
...
func Sign_evaluate(params ckks.Parameters, rlk *rlwe.RelinearizationKey, ctIn *rlwe.Ciphertext, signfunc SignComposition) (ctOut *rlwe.Ciphertext, err error) {
	...
	for i := 0; i < signfunc.ComponentNum; i++ {
		poly := ckks.NewPoly(signfunc.Components[i][:])
...
func Compare_evaluate(params ckks.Parameters, rlk *rlwe.RelinearizationKey, ctIn *rlwe.Ciphertext, signfunc SignComposition) (ctOut *rlwe.Ciphertext, err error) {
	evaluator := ckks.NewEvaluator(params, rlwe.EvaluationKey{Rlk: rlk})
	var ctSign *rlwe.Ciphertext
	ctSign, err = Sign_evaluate(params, rlk, ctIn, signfunc)
	if err != nil {
