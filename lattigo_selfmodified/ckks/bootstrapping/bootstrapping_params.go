...
// Number of bytes needed for MarshalBinary encoding of the target Parameters.
func (p *Parameters) MarshalBinarySize() (bytes int) {
	bytes = 0
	bytes += (p.SlotsToCoeffsParameters.MarshalBinarySize() + 1)
	bytes += (p.EvalModParameters.MarshalBinarySize() + 1)
	bytes += (p.CoeffsToSlotsParameters.MarshalBinarySize() + 1)
	bytes += 4
	return
}
